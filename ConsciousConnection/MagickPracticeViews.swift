...
private struct MagickPracticeRunnerView: View {
    ...
    private func practiceContent(
        ...
        if item.step == .traditionalSymbol {
            ...
            TraditionalSymbolPracticeView(symbol: symbol, geometry: geometry)
        ...
        ZStack {
            ...
            VStack(spacing: 28) {
                ...
                Image(symbol.assetName)
                    ...
                    .scaledToFit()
                    .frame(
                        maxWidth: min(geometry.size.width * 0.82, 440),
                ...
                Text("Let your gaze rest on the symbol. Notice its shape, feeling and associations without forcing an interpretation.")
                    .font(.system(size: 19, weight: .regular, design: .serif))
                    .multilineTextAlignment(.center)
                    .foregroundStyle(symbol.foregroundColor.opacity(0.92))
                    .padding(.horizontal, 38)
                    .padding(.bottom, 104)
            }
            .padding(.top, max(42, geometry.safeAreaInsets.top + 22))
        }
        .frame(width: geometry.size.width, height: geometry.size.height)
    }
}
